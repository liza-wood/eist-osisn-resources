...
m_null_fx <- function(netn){
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges,
...
m_design_fx <- function(netn, gwd_decay, esp_decay){
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges +
                    gwdegree(", gwd_decay, ", fixed = T) +
                    gwesp(", esp_decay, ", fixed = T) +
                    nodefactor('respondent', base = 2) +
                    nodecov('n_respondents_in_org') +
                    nodecov('generic_count') +
                    offset(edgecov(matrices[[", netn, "]])),
                    offset.coef = -Inf,
                    control = control.ergm(seed = 21, parallel = 4,
                                           MCMLE.termination = 'Hummel',
                                           MCMLE.effectiveSize = NULL))")))
...
m_chall_fx <- function(netn, gwd_decay, esp_decay){
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges +
                    gwdegree(", gwd_decay, ", fixed = T) +
                    gwesp(", esp_decay, ", fixed = T) +
                    nodematch('k_chall_avg') +
                    nodematch('v_chall_avg') +
                    nodefactor('respondent', base = 2) +
                    nodecov('n_respondents_in_org') +
                    nodecov('generic_count') +
                    offset(edgecov(matrices[[", netn, "]])),
...
m_geo_fx <- function(netn, gwd_decay, esp_decay, rolen){
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges + 
                    gwdegree(", gwd_decay, ", fixed = T) +
                    gwesp(", esp_decay, ", fixed = T) +
                    nodefactor('role_refined', base = ", rolen, ") +
                    nodematch('geo_scale', diff = T, levels = c(5:6,11:13)) +
...
# level and homophily is better fitting than having region as nodefactor

#edgecov(network(", hor_mat, ", direct = F)) +
# Everything but actors?
m_processes_fx <- function(netn, gwd_decay, esp_decay, hor_mat){
  hornet <- network(hor_mat, directed = F)
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges + 
                    gwdegree(", gwd_decay, ", fixed = T) +
...
#edgecov(network(", hor_mat, ", direct = F)) +
m_full_fx <- function(netn, gwd_decay, esp_decay, rolen, hor_mat){
  hornet <- network(hor_mat, directed = F)
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges + 
...
# Right now no offset
m_full_rm_fx <- function(net, gwd_decay, esp_decay, rolen, geoscalerange){
  model <- eval(parse(text = paste0(
    "ergm(", net, " ~ 
                    edges + 
                    gwdegree(", gwd_decay, ", fixed = T) +
                    gwesp(", esp_decay, ", fixed = T) +
                    nodefactor('role_refined', base = ", rolen, ") +
                    nodematch('geo_scale', diff = T, levels = c(", geoscalerange,")) +
                    nodefactor('lvl', base = 1) +
                    nodefactor('respondent', base = 2) +
                    #nodecov('n_respondents_in_org') + rm because it is not relevant
                    nodecov('generic_count'),
                    control = control.ergm(seed = 21, parallel = 4,
                                           MCMLE.termination = 'Hummel',
                                           MCMLE.effectiveSize = NULL))")))
...
m_full_exp_fx <- function(netn, gwd_decay, esp_decay, rolen, hor_mat){
  hornet <- network(hor_mat, directed = F)
  model <- eval(parse(text = paste0(
    "ergm(networks[[", netn, "]] ~ 
                    edges + 
